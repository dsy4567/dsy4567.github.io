...
fetch("/json/theme.json")
	...
	.catch(e => console.error(e));
fetch("https://dsy4567.eu.org/api/hitokoto")
	.then(res => res.json())
	.then(j => {
		const f = () => {
			let 一言 = gd("一言", true),
...
(() => {
	const f = async () => {
		...
		[gd("电子邮箱"), gd("tg")].forEach(元素 => {
            ...
            <a href="https://qwq.dsy4567.eu.org/api/getemail">在新标签页验证</a><br/>
            ...
            发送广告/要饭/雇童工/炒币等垃圾邮件。`,
				...
				添加脚本("https://www.recaptcha.net/recaptcha/api.js?render=explicit", null)
					.then(() => {
						...
						gd("recaptcha")?.addEventListener("click", async 事件 => {
							...
							try {
								...
								gr.innerHTML = await (
									await fetch(
										"https://qwq.dsy4567.eu.org/api/getemail?g-recaptcha-response=" +
											回复
									)
								).text();
								添加点击事件和设置图标({
					...
					.catch(() => {
						...
						open("https://qwq.dsy4567.eu.org/api/getemail", "_blank");
						div.remove();
					});
			});
		});
